...
def main(_run, _config, _log):
    ...
    log_loss = {'total_loss': 0, 'align_loss': 0, 'con_loss': 0, 'uncertainty_loss':0}
    ...
    for sub_epoch in range(n_sub_epochs):
        ...
        for _, sample in enumerate(train_loader):
            ...
            # enhance image
            support_images_enhance, support_fg_mask_enhance = image_enhance.Support_Enhance(support_images, support_fg_mask)
                
            # Compute outputs and losses.
            query_pred, align_loss, con_loss = model(support_images, support_fg_mask, support_images_enhance, support_fg_mask_enhance, query_images, train=True)
            
            query_uncertainty_map = -1 * torch.sum(query_pred * torch.log(query_pred + 1e-6), dim=1, keepdim=True)
            uncertainty_loss = weit_loss(query_pred, query_labels, query_uncertainty_map.squeeze(0))
            
            loss = align_loss + 0.5 * con_loss + uncertainty_loss
            ...
            con_loss = con_loss.detach().data.cpu().numpy()
            uncertainty_loss = uncertainty_loss.detach().data.cpu().numpy()

            _run.log_scalar('total_loss', loss.item())
            _run.log_scalar('con_loss', con_loss)
            _run.log_scalar('align_loss', align_loss)
            _run.log_scalar('uncertainty_loss', uncertainty_loss)

            log_loss['total_loss'] += loss.item()
            log_loss['align_loss'] += align_loss
            log_loss['con_loss'] += con_loss
            log_loss['uncertainty_loss'] += uncertainty_loss

            # Print loss and take snapshots.
            if (i_iter + 1) % _config['print_interval'] == 0:
                total_loss = log_loss['total_loss'] / _config['print_interval']
                align_loss = log_loss['align_loss'] / _config['print_interval']
                con_loss = log_loss['con_loss'] / _config['print_interval']
                uncertainty_loss = log_loss['uncertainty_loss'] / _config['print_interval']
                

                log_loss['total_loss'] = 0
                log_loss['align_loss'] = 0
                log_loss['con_loss'] = 0
                log_loss['uncertainty_loss'] = 0

                _log.info(f'step {i_iter + 1}: total_loss: {total_loss}'
                          f' align_loss: {align_loss}, con_loss:{con_loss}, uncertainty_loss:{uncertainty_loss}')
